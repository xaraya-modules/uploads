...
class FileMoveMethod extends MethodClass
{
    ...
    public function __invoke(array $args = [])
    {
        ...
        if (!is_readable($fileSrc)) {
            $msg = $this->ml('Unable to move file - Source file [#(1)]is unreadable!', $fileSrc);
            throw new Exception($msg);
        }

        if (!file_exists($fileSrc)) {
            $msg = $this->ml('Unable to move file - Source file [#(1)]does not exist!', $fileSrc);
            throw new Exception($msg);
        }
        ...
        if (!file_exists($dirDest)) {
            $msg = $this->ml('Unable to move file - Destination directory does not exist!');
            throw new Exception($msg);
        }

        if (!is_writable($dirDest)) {
            $msg = $this->ml('Unable to move file - Destination directory is not writable!');
            throw new Exception($msg);
        }

        $freespace = @disk_free_space($dirDest);
        if (!empty($freespace) && $freespace <= filesize($fileSrc)) {
            $msg = $this->ml('Unable to move file - Destination drive does not have enough free space!');
            throw new Exception($msg);
        }

        if (file_exists($fileDest) && $force != true) {
            $msg = $this->ml('Unable to move file - Destination file already exists!');
            throw new Exception($msg);
        }

        if ($isUpload) {
            if (!move_uploaded_file($fileSrc, $fileDest)) {
                $msg = $this->ml('Unable to move file [#(1)] to destination [#(2)].', $fileSrc, $fileDest);
                throw new Exception($msg);
            }
        } else {
